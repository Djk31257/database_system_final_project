    ...
    <form id="form1" runat="server">
        ...
        <table class="auto-style1">
            ...
            <asp:SqlDataSource ID="mealSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT mealTable.meal_id, mealTable.account, mealTable.date, mealTable.time, SUM(mealFoodTable.calories) AS TC, SUM(mealFoodTable.water) AS total_water, SUM(mealFoodTable.protein) AS total_protein, SUM(mealFoodTable.fat) AS total_fat, SUM(mealFoodTable.saturated_fat) AS total_saturated_fat, SUM(mealFoodTable.ash) AS total_ash, SUM(mealFoodTable.carbonhydrate) AS total_carbonhydrate, SUM(mealFoodTable.dietary_fiber) AS total_dietary_fiber, SUM(mealFoodTable.trans_fat) AS total_trans_fat FROM mealTable INNER JOIN mealFoodTable ON mealTable.meal_id = mealFoodTable.meal_id GROUP BY mealTable.meal_id, mealTable.account, mealTable.date, mealTable.time HAVING (mealTable.account = @account)" UpdateCommand="UPDATE mealTable SET date = @date, time = @time WHERE (meal_id = @meal_id)" DeleteCommand="DELETE FROM mealTable WHERE (meal_id = @meal_id)">
                ...
                <UpdateParameters>
                    <asp:ControlParameter Name="date" ControlID="mealGridView" PropertyName="SelectedValue" />
                    <asp:ControlParameter ControlID="mealGridView" Name="time" PropertyName="SelectedValue" />
                    <asp:ControlParameter ControlID="mealGridView" Name="meal_id" PropertyName="SelectedValue" />
                </UpdateParameters>
            </asp:SqlDataSource>
        <asp:GridView ID="mealGridView" runat="server" AllowSorting="True" AutoGenerateColumns="False" AutoGenerateSelectButton="True" DataKeyNames="meal_id" DataSourceID="mealSqlDataSource" CellPadding="4" ForeColor="#333333" GridLines="None" HorizontalAlign="Center" AutoGenerateEditButton="True" AutoGenerateDeleteButton="True" OnRowDeleting="mealGridView_RowDeleting">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
                <asp:TemplateField HeaderText="日期" SortExpression="date">
                    <EditItemTemplate>
                        <asp:Calendar ID="editDateCalendar" runat="server" SelectedDate='<%# Bind("date") %>'></asp:Calendar>
                    </EditItemTemplate>
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="時間" SortExpression="time">
                    <EditItemTemplate>
                        &nbsp;<asp:TextBox ID="timeTextBox" runat="server" Text='<%# Bind("time") %>' TextMode="Time"></asp:TextBox>
                    </EditItemTemplate>
                    <ItemTemplate>
                        <asp:Label ID="timeLabel" runat="server" Text='<%# Bind("time") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:BoundField DataField="TC" HeaderText="總熱量(大卡)" SortExpression="TC" DataFormatString="{0:N2}" ReadOnly="True" />
        ...
        <asp:SqlDataSource ID="mealFoodSqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT mealFood_id, meal_id, food_name, calories, weight, food_id, water, protein, fat, saturated_fat, ash, carbonhydrate, dietary_fiber, trans_fat FROM mealFoodTable WHERE (meal_id = @meal_id) AND (food_name &lt;&gt; @default )" InsertCommand="INSERT INTO mealFoodTable(meal_id, food_name, calories, weight, food_id, water, protein, fat, saturated_fat, ash, carbonhydrate, dietary_fiber, trans_fat) VALUES (@meal_id, @food_name, @weight * @unit_calorie / 100, @weight, @food_id, @weight * @unit_water / 100, @weight * @unit_protein / 100, @weight * @unit_fat / 100, @weight * @unit_saturated_fat / 100, @weight * @unit_ash / 100, @weight * @unit_carbonhydrate / 100, @weight * @unit_dietary_fiber / 100, @weight * @unit_trans_fat / 100)" DeleteCommand="DELETE FROM mealFoodTable WHERE (mealFood_id = @mealFood_id)" UpdateCommand="UPDATE mealFoodTable SET weight = @weight, calories = @weight * nutrientTable.calorie / 100, water = @weight * nutrientTable.water / 100, protein = @weight * nutrientTable.protein / 100, fat = @weight * nutrientTable.fat / 100, saturated_fat = @weight * nutrientTable.saturated_fat / 100, ash = @weight * nutrientTable.ash / 100, carbonhydrate = @weight * nutrientTable.carbonhydrate / 100, dietary_fiber = @weight * nutrientTable.dietary_fiber / 100, trans_fat = @weight * nutrientTable.trans_fat / 100 FROM mealFoodTable INNER JOIN nutrientTable ON mealFoodTable.food_id = nutrientTable.id WHERE (mealFoodTable.mealFood_id = @mealFood_id)">
            ...
            <UpdateParameters>
                <asp:ControlParameter ControlID="mealFoodGridView" Name="weight" PropertyName="SelectedValue" />
                <asp:ControlParameter ControlID="mealFoodGridView" Name="mealFood_id" PropertyName="SelectedValue" />
            </UpdateParameters>
        </asp:SqlDataSource>
        <asp:GridView ID="mealFoodGridView" runat="server" AutoGenerateColumns="False" CellPadding="4" DataKeyNames="mealFood_id" DataSourceID="mealFoodSqlDataSource" ForeColor="#333333" GridLines="None" HorizontalAlign="Center" AutoGenerateDeleteButton="True" OnRowDeleted="mealFoodGridView_RowDeleted" AllowSorting="True" AutoGenerateEditButton="True">
            <AlternatingRowStyle BackColor="White" />
            <Columns>
                <asp:TemplateField HeaderText="食物名稱" SortExpression="food_name">
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="重量" SortExpression="weight">
                    <EditItemTemplate>
                        <asp:TextBox ID="weightTextBox" runat="server" Text='<%# Bind("weight") %>' TextMode="Number"></asp:TextBox>
                    </EditItemTemplate>
                    <ItemTemplate>
                        <asp:Label ID="Label1" runat="server" Text='<%# Bind("weight") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="熱量" SortExpression="calories">
                    <ItemTemplate>
                        <asp:Label ID="Label3" runat="server" Text='<%# Bind("calories", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="水" SortExpression="water">
                    <ItemTemplate>
                        <asp:Label ID="Label4" runat="server" Text='<%# Bind("water", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="蛋白質" SortExpression="protein">
                    <ItemTemplate>
                        <asp:Label ID="Label5" runat="server" Text='<%# Bind("protein", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="脂肪" SortExpression="fat">
                    <ItemTemplate>
                        <asp:Label ID="Label6" runat="server" Text='<%# Bind("fat", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="飽和脂肪" SortExpression="saturated_fat">
                    <ItemTemplate>
                        <asp:Label ID="Label7" runat="server" Text='<%# Bind("saturated_fat", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="反式脂肪" SortExpression="trans_fat">
                    <ItemTemplate>
                        <asp:Label ID="Label8" runat="server" Text='<%# Bind("trans_fat", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="碳水化合物" SortExpression="carbonhydrate">
                    <ItemTemplate>
                        <asp:Label ID="Label9" runat="server" Text='<%# Bind("carbonhydrate", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="膳食纖維" SortExpression="dietary_fiber">
                    <ItemTemplate>
                        <asp:Label ID="Label10" runat="server" Text='<%# Bind("dietary_fiber", "{0:N2}") %>'></asp:Label>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
        ...
        <table class="auto-style1">
            <tr>
                <td class="auto-style6">
                    <asp:Label ID="nutrientLabel" runat="server" Text="這個食物每100g含有營養：" style="font-size: large"></asp:Label>
                </td>
            </tr>
        </table>
